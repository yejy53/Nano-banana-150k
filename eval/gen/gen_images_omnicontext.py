...
def move_generation_input_to_device(generation_input, device):
    """Move all tensors in generation_input to the specified device"""
        ...
        for key, value in generation_input.items():
            if isinstance(value, torch.Tensor):
                ...
                if len(value) > 0 and isinstance(value[0], torch.Tensor):
                    generation_input[key] = [v.to(device) if isinstance(v, torch.Tensor) else v for v in value]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--image_dir', type=str, required=True, help="dir to the input images.")
    ...
    try:
        ...
        base_model_state_dict_path = os.path.join(args.model_path, "ema.safetensors")
        base_model_state_dict = load_file(base_model_state_dict_path, device="cpu")
        msg_base = model.load_state_dict(base_model_state_dict, strict=False)
        ...
        device_new_token_ids = {}
        for key, value in new_token_ids.items():
            if isinstance(value, torch.Tensor):
                device_new_token_ids[key] = value.to(device)
            else:
                device_new_token_ids[key] = value
        new_token_ids = device_new_token_ids
        ...
        for idx in range(start, end):
            ...
            if isinstance(metadata['ori_img'], str):
                images.append(pil_img2rgb(Image.open(os.path.join(args.image_dir,metadata['task_type'], metadata['ori_img']))))
            else:
                for img_path in metadata['ori_img']:
                    images.append(pil_img2rgb(Image.open(os.path.join(args.image_dir,metadata['task_type'], img_path))))
            prompt = metadata['ins_en']
            os.makedirs(os.path.join(output_dir,metadata['task_type']), exist_ok=True)
            outpath = os.path.join(output_dir,metadata['task_type'], f"{metadata['id']}.png")
