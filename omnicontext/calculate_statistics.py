...
if __name__ == "__main__":
    ...
    result_json_files = glob.glob(os.path.join(args.result_dir, args.backbone, "**/*.jsonl"))
    ...
    for result_json_file in result_json_files:
        with open(result_json_file, 'r') as f:
            for line in f:
                ...
                if task_type == "single_character" or task_type == "single_object":
                    continue
    ...
    for task_type in group_prompt_following_scores.keys():
        ...
        result[task_type] = {
            "PF_score": group_prompt_following_scores[task_type],
            "SC_score": group_subject_consistency_scores[task_type],
            "overall_score": group_overall_scores[task_type]
        }
    ...
    result["average"] = {
        "PF_score": np.mean(list(group_prompt_following_scores.values())),
        "SC_score": np.mean(list(group_subject_consistency_scores.values())),
        "overall_score": np.mean(list(group_overall_scores.values()))
    }
    # Save the results to a JSON file
    save_path = os.path.join(args.save_path,'statistics.json')
